	...
	<view class="user">
		<view class="header">
			...
			<view class="header-line" :style="{height: '1px','background-color': navBg}"></view>
			 <u-navbar :is-back="false" title="个人中心" :title-bold="true" :border-bottom="false" :background="{ background: navBg }" :title-color="navC"></u-navbar>
			 <!-- #endif -->
			<view class="user-info row-between">
				<view class="info row">
					<image class="avatar mr20" @tap="goLogin" :src="isLogin ? userInfo.avatar : '/static/images/my_portrait_empty.png'"></image>
...
<script>
	...
	import {menuJump, copy, goToPay} from '@/utils/tools'
	...
	export default {
		...
		computed: {
			...mapGetters(["cartNum", "userInfo", "inviteCode"])
		}
	};
</script>
<style lang="scss">
