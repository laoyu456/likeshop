						...
						<view class="info" style="flex: 1">
							...
							<view class="row-between mt20">
								...
								<view class="cartNum" @tap.stop="">
									<u-number-box :value="item.goods_num" :min="1" :max="99" integer @change="countChange($event, item.cart_id)" />
								</view>
							</view>
						</view>
					</view>
	...
	export default {
		...
		methods: {
			...
			goToConfirm() {
				...
				// 解决iosH5支付路径问题
				// #ifdef H5
				location.href = '/mobile/pages/confirm_order/confirm_order?data=' + encodeURIComponent((JSON.stringify({goods, type:'cart'})))
				// #endif
			},
